...
export const loginWithGoogle = async (req, res) => {
  try {
    ...
    // Création des 3 comptes si inexistants
...
export const verifyEmail2FA = async (req, res) => {
  try {
    ...
    const token = generateToken(user._id);
    res.json({ message: "2FA vérifié", token, user: { id: user._id, name: user.name, email: user.email, role: user.role } });
  } catch (err) {
...
export const resetPassword = async (req, res) => {
  try {
    ...
    res.json({ message: "Mot de passe réinitialisé avec succès !", token: jwtToken, user: { id: user._id, name: user.name, email: user.email, role: user.role } });
  } catch (err) {
