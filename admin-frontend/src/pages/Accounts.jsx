...
export default function Accounts() {
  ...
  const handleTransaction = async (type) => {
    ...
    try {
      await api("POST", `/accounts/${selectedAccount}/${type}`, { amount });
    ...
    <div className="flex min-h-screen bg-[#f5f2ee]">
      <div className="flex-1 p-4">
        ...
        <div className="mb-4 flex flex-col md:flex-row gap-2 items-center">
          ...
          <button className="bg-[#432703] text-white px-4 py-2 rounded hover:bg-[#a28870]" onClick={() => handleTransaction("deposit")}>
            Déposer
          </button>
          <button className="bg-[#a28870] text-white px-4 py-2 rounded hover:bg-[#432703]" onClick={() => handleTransaction("withdraw")}>
            Retirer
          </button>
        </div>
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {filteredAccounts.length === 0 && (
                <tr>
                  <td colSpan={4} className="text-center p-4 text-gray-500">Aucun compte trouvé</td>
                </tr>
              )}
